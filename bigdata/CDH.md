...
>
> 注意大数据集群本身就很吃内存，没有32G不要尝试部署三台集群，16G跑两台就行了（8G + 4G分配就行），还有虚拟机不要乱设置，你有多少cpu核心就按照具体情况分配，例如4核CPU，一台给4核另一台给2两核就行（多一点没关系，毕竟超线程是吧）。

配置集群主机名

**以后配置主机名都不要再加`_`或`-`等特殊符号了，框架可能不支持（cdh不支持带下划线的主机名）**

**如果你改了hostname，记得更改其他先关的配置！！！**
...
这里为了方便，我在windows的docker上部署mysql（懒得在linux折腾mysql了）
...
> #### 如果你是想上面那样做，那就上当了。（艹，后面发现cdh有些东西部署，建议在集群主机上部署（更有些只能在集群主机上部署，直接折磨，搞到最后发现又要装一个mysql））



> 这里也没什么别的，网上搜一篇帖子，虚拟机装一个mysql就行了，也可以在虚拟机里装一个docker，能在虚拟机跑mysql就行。
...
4. 集群 **Install Parcels**阶段如果出现`主机状况不良`，可能是虚拟机之前运行过cdh，出现了主机内主节点不一致的情况，将出现问题的agent节点下的`/var/lib/cloudera-scm-agent/cm_guid`删除即可
